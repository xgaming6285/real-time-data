...
export function TradingSidebar({
  ...
  const handlePlaceOrder = useCallback(async (type: 'buy' | 'sell') => {
    ...
    // Frontend validation for Stop Loss
    if (slValue !== null) {
      if (type === 'buy' && slValue >= entryPrice) {
        setOrderError('For BUY orders, Stop Loss must be below entry price');
        setOrderLoading(false);
        return;
      }
      if (type === 'sell' && slValue <= entryPrice) {
        setOrderError('For SELL orders, Stop Loss must be above entry price');
        setOrderLoading(false);
        return;
      }
    }

    // Frontend validation for Take Profit
    if (tpValue !== null) {
      if (type === 'buy' && tpValue <= entryPrice) {
        setOrderError('For BUY orders, Take Profit must be above entry price');
        setOrderLoading(false);
        return;
      }
      if (type === 'sell' && tpValue >= entryPrice) {
        setOrderError('For SELL orders, Take Profit must be below entry price');
        setOrderLoading(false);
        return;
      }
    }
    ...
    setOrderLoading(false);
  }, [symbol, volume, currentAsk, currentBid, stopLoss, takeProfit, placeOrder]);

  const handleCloseOrder = useCallback(async (order: Order) => {
    const closePrice = order.type === 'buy' ? currentBid : currentAsk;
    const result = await closeOrder(order._id, closePrice);
    
    if (!result.success) {
      setOrderError(result.error || 'Failed to close order');
    }
  }, [currentBid, currentAsk, closeOrder]);
  ...
  const totalUnrealizedPnL = orders.reduce((sum, order) => sum + calculateProfit(order), 0);
  ...
  const realMarginLevel = account && account.margin > 0 ? (realEquity / account.margin) * 100 : 0;
  ...
  return (
    <>
      ...
      >
        <svg 
          className={`w-4 h-4 text-white/70 group-hover:text-white transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
          fill="none" 
          viewBox="0 0 24 24" 
          stroke="currentColor"
        >
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[380px] z-40
          bg-gradient-to-b from-[#1a1a24]/95 to-[#0d0d12]/98
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          ...
          <div className="px-5 py-4 border-b border-white/5">
            <h3 className="text-xs font-medium text-white/40 uppercase tracking-wider mb-3">Account</h3>
            ...
            {accountLoading ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/5 rounded-lg p-3">
                  ...
                  <div className="text-sm font-semibold text-white">{formatMoney(account.balance)}</div>
                </div>
                <div className="bg-white/5 rounded-lg p-3">
                  <div className="text-xs text-white/40 mb-1">Equity</div>
                  <div className={`text-sm font-semibold ${realEquity >= account.balance ? 'text-(--accent-green)' : 'text-(--accent-red)'}`}>
                    {formatMoney(realEquity)}
                  </div>
                </div>
                <div className="bg-white/5 rounded-lg p-3">
                  <div className="text-xs text-white/40 mb-1">Free Margin</div>
                  <div className="text-sm font-semibold text-white">{formatMoney(realFreeMargin)}</div>
                </div>
                <div className="bg-white/5 rounded-lg p-3">
                  <div className="text-xs text-white/40 mb-1">Margin Level</div>
                  <div className="text-sm font-semibold text-white">
                    {realMarginLevel > 0 ? `${realMarginLevel.toFixed(0)}%` : '—'}
                  </div>
                </div>
              </div>
            ) : (
          ...
          {/* Quick Trade Panel */}
          <div className="px-5 py-4 border-b border-white/5">
            <h3 className="text-xs font-medium text-white/40 uppercase tracking-wider mb-3">Quick Trade</h3>
            
            {/* Price Display */}
            <div className="flex gap-2 mb-4">
              <div className="flex-1 bg-gradient-to-br from-red-500/10 to-red-600/5 rounded-lg p-3 border border-red-500/20">
                <div className="text-xs text-red-400/70 mb-0.5">SELL (Bid)</div>
                <div className="text-xl font-bold text-red-400 tabular-nums">{formatPrice(currentBid)}</div>
              </div>
              <div className="flex-1 bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 rounded-lg p-3 border border-emerald-500/20">
                <div className="text-xs text-emerald-400/70 mb-0.5">BUY (Ask)</div>
                <div className="text-xl font-bold text-emerald-400 tabular-nums">{formatPrice(currentAsk)}</div>
              </div>
            </div>
            ...
            <div className="text-center mb-4">
              ...
              <span className="text-xs font-medium text-white/70">{spreadPips.toFixed(1)} pips</span>
            </div>

            {/* Volume Input */}
            <div className="mb-4">
              <label className="block text-xs text-white/40 mb-1.5">Volume (Lots)</label>
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => setVolume(v => Math.max(0.01, parseFloat(v) - 0.01).toFixed(2))}
                ...
                <input
                  ...
                  min="0.01"
                  step="0.01"
                  className="flex-1 h-10 bg-white/5 border border-white/10 rounded-lg px-3 text-center text-white font-medium focus:border-(--accent-cyan) focus:outline-none"
                />
                <button 
                  onClick={() => setVolume(v => (parseFloat(v) + 0.01).toFixed(2))}
                  className="w-10 h-10 rounded-lg bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-colors flex items-center justify-center"
                >
                  +
                </button>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                <label className="block text-xs text-white/40 mb-1.5">Stop Loss</label>
                <input
                  type="number"
                  value={stopLoss}
                  ...
                  step="0.00001"
                  className="w-full h-9 bg-white/5 border border-white/10 rounded-lg px-3 text-sm text-white placeholder-white/30 focus:border-(--accent-red) focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs text-white/40 mb-1.5">Take Profit</label>
                <input
                  type="number"
                  value={takeProfit}
                  ...
                  placeholder="Optional"
                  step="0.00001"
                  className="w-full h-9 bg-white/5 border border-white/10 rounded-lg px-3 text-sm text-white placeholder-white/30 focus:border-(--accent-green) focus:outline-none"
                />
              </div>
            </div>
          ...
          <div className="px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-medium text-white/40 uppercase tracking-wider">Open Positions</h3>
              <span className="text-xs text-white/30">{orders.length} open</span>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs text-white/40">{order.volume} lots</span>
                      </div>
